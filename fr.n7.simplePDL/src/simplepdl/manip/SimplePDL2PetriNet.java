	...
	public static void main (String[] args) {
	    ...
	    for (ProcessElement pe : process.getProcessElements()) {
	    	...
	    	if (pe instanceof WorkDefinition) {
	    		...
	    		for (RessourceNeeded rn : wd.getRessources()) {
	    			Place p = null;
		    		for(Place tmp : lpl) {
		    		...
		    		if (p==null) {
		    			p = myFactory.createPlace();
		    			lpl.add(p);
		    			p.setName(rn.getRessource().getName());
		    			p.setWeight(rn.getRessource().getQuantity());
		    			
		    		}
		    		// Creation of two links : one to take ressources, and one to return the ressource at the end
		    		Link takeRessource = myFactory.createLink();
		    		Link giveRessource = myFactory.createLink();
		    		lli.add(takeRessource);
		    		lli.add(giveRessource);
		    		takeRessource.setPlace(p);
		    		takeRessource.setTransition(trStart);
		    		takeRessource.setPositive(false);
		    		takeRessource.setWeight(rn.getQuantity());
		    		giveRessource.setPlace(p);
		    		giveRessource.setTransition(trFinish);
		    		giveRessource.setPositive(true);
		    		giveRessource.setWeight(rn.getQuantity());
	    		}
	    	}
	    }
	    for (ProcessElement pe : process.getProcessElements()) {
	    	if (pe instanceof WorkSequence) {
	    		...
	    		Ressource r = (Ressource) pe;
	    		Place p = myFactory.createPlace();
    			lpl.add(p);
    			p.setName(r.getName());
    			p.setWeight(r.getQuantity());
	    	}
	    }
	    for (Place p : lpl) {
